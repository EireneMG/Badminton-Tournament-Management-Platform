...
return new class extends Migration
{
    ...
    public function down(): void
    {
        if (DB::getDriverName() === 'sqlite') {
            DB::statement("
                CREATE TABLE tournament_registrations_old (
                    id INTEGER PRIMARY KEY AUTOINCREMENT,
                    tournament_id INTEGER NOT NULL,
                    player_id INTEGER NOT NULL,
                    category_id INTEGER,
                    partner_id INTEGER,
                    status TEXT CHECK(status IN ('pending_payment', 'paid', 'approved', 'rejected', 'withdrawn')) DEFAULT 'pending_payment',
                    payment_proof TEXT,
                    payment_verified_at DATETIME,
                    created_at DATETIME,
                    updated_at DATETIME,
            ...
            DB::statement("
                INSERT INTO tournament_registrations_old (id, tournament_id, player_id, category_id, partner_id, status, payment_verified_at, created_at, updated_at)
                SELECT id, tournament_id, player_id, category_id, partner_id,
                    CASE 
                        WHEN status IN ('pending', 'eligible', 'awaiting_payment') THEN 'pending_payment'
                        ELSE status
                    END,
                    payment_verified_at, created_at, updated_at
                FROM tournament_registrations
            ...
            DB::statement("DROP TABLE tournament_registrations");
            DB::statement("ALTER TABLE tournament_registrations_old RENAME TO tournament_registrations");
        } else {
            Schema::table('tournament_registrations', function (Blueprint $table) {
                $table->string('payment_proof')->nullable()->after('status');
            });
            
            DB::statement("ALTER TABLE tournament_registrations MODIFY COLUMN status ENUM('pending_payment', 'paid', 'approved', 'rejected', 'withdrawn') DEFAULT 'pending_payment'");
        }
    }
};
